const sdk = require('node-appwrite');

const client = new sdk.Client();
client
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('viscareelojavirtual1610')
    .setKey('standard_');

const databases = new sdk.Databases(client);
const DB_ID = '6a390e430024feb8df57';
const COLLECTION_ID = 'products';

async function updateAttributeSize() {
    try {
        console.log('Updating description_pt to 66000 (forces MEDIUMTEXT)...');
        await databases.updateStringAttribute(DB_ID, COLLECTION_ID, 'description_pt', false, null, 66000);
        console.log('description_pt updated.');
    } catch(e) { console.error(e.message); }

    try {
        console.log('Updating description_it to 66000...');
        await databases.updateStringAttribute(DB_ID, COLLECTION_ID, 'description_it', false, null, 66000);
        console.log('description_it updated.');
    } catch(e) { console.error(e.message); }
}

updateAttributeSize();
