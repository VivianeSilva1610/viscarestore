    ...
    <div className="min-h-screen bg-[#F1E7E2] flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-4 py-12">
        <div className="bg-white rounded-3xl shadow-sm border border-neutral-100 p-8 md:p-12 max-w-lg w-full text-center">
          ...
          <div className="bg-[#F8F5F2] rounded-2xl p-5 border border-[#C8A97E]/15 mb-8 text-left space-y-3">
            <div className="flex items-center gap-3">
              <Package size={20} className="text-[#C8A97E]" />
              <div>
                <p className="text-[10px] uppercase tracking-widest text-neutral-400 font-semibold mb-0.5">Código de Rastreio</p>
                <p className="text-sm text-neutral-800 font-medium">{trackingCode || "Processando..."}</p>
              </div>
            </div>
            {estimatedDeliveryDate && (
              <p className="text-xs text-neutral-500 pl-1">
                Previsão de entrega: {new Date(estimatedDeliveryDate).toLocaleDateString("pt-BR", { day: "numeric", month: "long" })}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            {trackingCode && (
              <button
                onClick={() => router.push(`/rastreio?code=${trackingCode}`)}
