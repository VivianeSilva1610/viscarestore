...
export default function GuiaNotasOlfativas() {
  ...
  const content = {
    pt: {
      ...
      suggestion: {
        title: "Sugestão para a VisCaree",
        text: "Você pode adicionar em cada perfume um selo visual:",
        seals: [
          "🍋 Fresco", "🌸 Floral", "🍦 Doce", "🌲 Amadeirado",
          "🔥 Intenso", "🌊 Aquático", "🍓 Frutado", "🌿 Aromático"
        ],
        footer: "Assim os clientes identificam rapidamente o estilo da fragrância e compram com mais confiança."
      }
    },
    it: {
      ...
      suggestion: {
        title: "Suggerimento per VisCaree",
        text: "Puoi aggiungere a ogni profumo un sigillo visivo:",
        seals: [
          "🍋 Fresco", "🌸 Floreale", "🍦 Dolce", "🌲 Legnoso",
          "🔥 Intenso", "🌊 Acquatico", "🍓 Fruttato", "🌿 Aromatico"
        ],
        footer: "Così i clienti identificano rapidamente lo stile della fragranza e acquistano con maggiore sicurezza."
      }
    }
  };
            ...
            <div className="max-w-4xl mx-auto px-6 sm:px-12 lg:px-16">
              ...
              <div className="space-y-6 mb-16">
                {t.families.map((family, index) => (
                  <div key={index} className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-neutral-100 flex flex-col md:flex-row gap-8 items-center transition-all hover:shadow-md hover:border-dourado-suave/30">
                    <div className="w-24 h-24 shrink-0 bg-[#F1E7E2]/50 rounded-full flex items-center justify-center text-5xl">
                      {family.emoji}
                    </div>
                    <div className="flex-1">
                      <h3 className="font-serif-premium text-2xl text-neutral-900 mb-4 text-center md:text-left">{family.title}</h3>
                          ...
                          <ul className="space-y-1">
                            {family.characteristics.map((char, i) => (
                              <li key={i} className="font-sans-premium text-xs text-neutral-600 flex items-center gap-2">
                                <span className="w-1 h-1 bg-dourado-suave rounded-full"></span>
                                {char}
                              </li>
                            ))}
                          </ul>
              ...
              {/* VisCaree Suggestion */}
              <section className="bg-gradient-to-br from-[#E7D8D0]/50 to-white p-8 md:p-10 rounded-3xl border border-dourado-suave/20 text-center">
                <h3 className="font-serif-premium text-xl text-neutral-900 mb-3">{t.suggestion.title}</h3>
                <p className="font-sans-premium text-sm text-neutral-600 mb-6">{t.suggestion.text}</p>
                <div className="flex flex-wrap justify-center gap-3 mb-6">
                  {t.suggestion.seals.map((seal, i) => (
                    <span key={i} className="bg-white border border-dourado-suave/30 text-neutral-800 font-sans-premium text-[11px] font-semibold tracking-wider px-4 py-2 rounded-full shadow-sm">
                      {seal}
                    </span>
                  ))}
                </div>
                <p className="font-sans-premium text-xs text-neutral-500 italic max-w-lg mx-auto">
                  "{t.suggestion.footer}"
                </p>
              </section>
