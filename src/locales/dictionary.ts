export const dictionary = {
  pt: {
    // Navbar
    nav: {
      novidades: "Novidades",
      perfumes: "Perfumes",
      skincare: "Skincare",
      rotina_skincare: "Rotina Skincare",
      minha_conta: "Minha Conta",
      bem_vinda: "Bem-vinda,",
      meu_perfil: "Meu Perfil",
      sair: "Sair da conta",
      entrar: "Entrar na conta",
      criar_conta: "Criar conta grátis"
    },
    // Hero
    hero: {
      curadoria: "Seleção com Critério",
      titulo1: "Perfumaria e skincare ",
      titulo_destaque: "escolhidos",
      titulo2: " a dedo, sem enrolação.",
      descricao: "Testamos, avaliamos ingredientes e só incluímos no catálogo o que realmente funciona — perfumes com fixação real e skincare com ativos comprovados.",
      botao: "Descobrir Coleção",
      sinergia: "Testado & Aprovado",
      edicao: "Selecionado este mês"
    },
    // Categories
    categories: {
      destaque: "O que selecionamos",
      sinergia: "Três categorias, um critério de escolha",
      explorar: "Explorar",
      items: {
        perfumes: {
          title: "Perfumaria",
          subtitle: "Selecionado pela fixação e qualidade",
          description: "Perfumes avaliados pela durabilidade em pele real, qualidade das matérias-primas e relação custo-benefício honesta — sem pagar pela embalagem.",
          details: "Notas Olfativas: Floral Amadeirado Especiado"
        },
        skincare: {
          title: "Cosméticos & Skincare",
          subtitle: "Ingredientes com propósito",
          description: "Produtos com ativos comprovados — ácido hialurônico, retinol, vitamina C — escolhidos com base em eficácia real e avaliações de quem usou, não em jargão técnico.",
          details: "Ativos: Hidratação, Firmeza & Uniformização"
        },
        vegano: {
          title: "Beleza que cuida de você e do mundo",
          subtitle: "Skincare & Perfumaria Vegana",
          description: "Todo o nosso catálogo é vegano e cruelty free — ingredientes naturais, escolhas conscientes e sustentabilidade em cada produto, do skincare à perfumaria.",
          details: "Vegano · Cruelty Free · Ingredientes Naturais · Sustentável"
        }
      }
    },
    // Product Grid
    grid: {
      colecao_modelo: "Coleção Modelo",
      selecao_pensada: "Uma seleção pensada para você",
      todos: "Todos",
      perfumes: "Perfumes",
      acessorios: "Acessórios",
      skincare: "Skincare",
      esgotado: "ESGOTADO",
      adicionar_sacola: "Adicionar à Sacola",
      nenhum_produto: "Nenhum produto encontrado nesta categoria."
    },
    // Footer
    footer: {
      entrega_vip: "Entrega Rastreada",
      entrega_desc: "Envio com rastreio em tempo real para o Brasil e para a Europa.",
      autenticidade: "Produtos Verificados",
      autenticidade_desc: "Cada produto no catálogo passou pela nossa avaliação antes de ser publicado.",
      troca_facilitada: "Troca Facilitada",
      troca_desc: "Solicitação de troca ou devolução em até 7 dias após o recebimento.",
      atendimento_exclusivo: "Atendimento Direto",
      atendimento_desc: "Suporte via WhatsApp — dúvidas sobre fragrâncias, tamanhos ou pedidos.",
      sobre_marca: "Uma seleção de perfumaria e skincare escolhida com critério real — testada pela nossa equipe e escolhida pela qualidade dos ingredientes, não pelo nome da marca.",
      inscreva_se: "Inscreva-se no Club VisCaree",
      email_placeholder: "Seu endereço de e-mail",
      termos_newsletter: "Ao se inscrever, você concorda com nossa Política de Privacidade.",
      colecoes: "Coleções",
      alta_perfumaria: "Alta Perfumaria",
      skincare_biologico: "Skincare Biológico",
      informacoes: "Informações",
      sobre_viscaree: "Sobre a VisCaree",
      nossa_missao: "Nossa Missão",
      app_skincare: "App Skincare (Cuidado)",
      contato: "Contato",
      ajuda_suporte: "Ajuda & Suporte",
      duvidas_frequentes: "Dúvidas Frequentes",
      politicas_frete: "Políticas de Frete",
      devolucoes_trocas: "Devoluções & Trocas",
      guia_notas: "Guia de Notas Olfativas",
      tabela_medidas: "Tabela de Medidas",
      direitos: "© 2026 VisCaree. Todos os direitos reservados. CNPJ: 00.000.000/0001-00.",
      termos_uso: "Termos de Uso",
      politicas_privacidade: "Políticas de Privacidade"
    }
  },
  it: {
    // Navbar
    nav: {
      novidades: "Novità",
      perfumes: "Profumi",
      skincare: "Skincare", // User requested NOT to change "skincare"
      rotina_skincare: "Routine Skincare",
      minha_conta: "Il Mio Account",
      bem_vinda: "Benvenuta,",
      meu_perfil: "Il Mio Profilo",
      sair: "Esci",
      entrar: "Accedi",
      criar_conta: "Crea account gratuito"
    },
    // Hero
    hero: {
      curadoria: "Selezione con Criterio",
      titulo1: "Profumeria e skincare ",
      titulo_destaque: "scelti",
      titulo2: " con cura, senza esagerazioni.",
      descricao: "Testiamo, valutiamo gli ingredienti e inseriamo nel catalogo solo ciò che funziona davvero — profumi con reale durata e skincare con attivi comprovati.",
      botao: "Scopri la Collezione",
      sinergia: "Testato & Approvato",
      edicao: "Selezionato questo mese"
    },
    // Categories
    categories: {
      destaque: "Cosa selezioniamo",
      sinergia: "Tre categorie, un unico criterio di scelta",
      explorar: "Esplora",
      items: {
        perfumes: {
          title: "Profumeria",
          subtitle: "Scelto per durata e qualità",
          description: "Profumi valutati per la durata sulla pelle reale, la qualità delle materie prime e il giusto rapporto qualità-prezzo — senza pagare per la confezione.",
          details: "Note Olfattive: Floreale Legnoso Speziato"
        },
        skincare: {
          title: "Cosmetici & Skincare",
          subtitle: "Ingredienti con uno scopo",
          description: "Prodotti con attivi comprovati — acido ialuronico, retinolo, vitamina C — scelti in base all'efficacia reale e alle recensioni di chi li ha usati, non al gergo tecnico.",
          details: "Attivi: Idratazione, Compattezza & Uniformazione"
        },
        vegano: {
          title: "Bellezza che prende cura di te e del mondo",
          subtitle: "Skincare & Profumeria Vegana",
          description: "Tutto il nostro catalogo è vegano e cruelty free — ingredienti naturali, scelte consapevoli e sostenibilità in ogni prodotto, dallo skincare alla profumeria.",
          details: "Vegano · Cruelty Free · Ingredienti Naturali · Sostenibile"
        }
      }
    },
    // Product Grid
    grid: {
      colecao_modelo: "Collezione Modello",
      selecao_pensada: "Una selezione pensata per te",
      todos: "Tutti",
      perfumes: "Profumi",
      acessorios: "Accessori",
      skincare: "Skincare", // Exceção
      esgotado: "ESAURITO",
      adicionar_sacola: "Aggiungi al Carrello",
      nenhum_produto: "Nessun prodotto trovato in questa categoria."
    },
    // Footer
    footer: {
      entrega_vip: "Spedizione Tracciata",
      entrega_desc: "Invio con tracciamento in tempo reale per il Brasile e per l'Europa.",
      autenticidade: "Prodotti Verificati",
      autenticidade_desc: "Ogni prodotto nel catalogo è stato valutato dal nostro team prima di essere pubblicato.",
      troca_facilitada: "Reso Facile",
      troca_desc: "Richiesta di reso o cambio entro 7 giorni dalla ricezione.",
      atendimento_exclusivo: "Assistenza Diretta",
      atendimento_desc: "Supporto via WhatsApp — domande su fragranze, taglie o ordini.",
      sobre_marca: "Una selezione di profumeria e skincare scelta con criterio reale — testata dal nostro team e selezionata per la qualità degli ingredienti, non per il nome del brand.",
      inscreva_se: "Iscriviti al Club VisCaree",
      email_placeholder: "Il tuo indirizzo email",
      termos_newsletter: "Iscrivendoti, accetti la nostra Informativa sulla Privacy.",
      colecoes: "Collezioni",
      alta_perfumaria: "Alta Profumeria",
      skincare_biologico: "Skincare Biologico",
      informacoes: "Informazioni",
      sobre_viscaree: "Chi è VisCaree",
      nossa_missao: "La Nostra Missione",
      app_skincare: "App Skincare (Cura)",
      contato: "Contatti",
      ajuda_suporte: "Aiuto & Supporto",
      duvidas_frequentes: "Domande Frequenti",
      politicas_frete: "Politiche di Spedizione",
      devolucoes_trocas: "Resi & Cambi",
      guia_notas: "Guida alle Note Olfattive",
      tabela_medidas: "Tabella Taglie",
      direitos: "© 2026 VisCaree. Tutti i diritti riservati. CNPJ: 00.000.000/0001-00.",
      termos_uso: "Termini di Utilizzo",
      politicas_privacidade: "Informativa sulla Privacy"
    }
  }
};
