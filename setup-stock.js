const sdk = require('node-appwrite');

const client = new sdk.Client();
client
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('viscareelojavirtual1610')
    .setKey('standard_');

const databases = new sdk.Databases(client);
const DB_ID = '6a390e430024feb8df57';
const COLLECTION_ID = 'products';

async function checkAndCreateAttributes() {
    try {
        console.log('Fetching existing attributes...');
        const res = await databases.listAttributes(DB_ID, COLLECTION_ID);
        const existingKeys = res.attributes.map(a => a.key);
        console.log('Existing attributes:', existingKeys.join(', '));

        if (!existingKeys.includes('stock_quantity')) {
            console.log('Creating missing attribute: stock_quantity...');
            try {
                // Integer attribute, required: false, min: 0, max: 1000000, default: 0
                await databases.createIntegerAttribute(DB_ID, COLLECTION_ID, 'stock_quantity', false, 0, 1000000, 0);
                console.log('Created stock_quantity');
            } catch(e) {
                console.error(`Failed to create stock_quantity:`, e.message);
            }
        } else {
            console.log('Attribute stock_quantity already exists.');
        }
        console.log('Finished checking attributes.');
    } catch(e) {
        console.error('Error:', e.message);
    }
}

checkAndCreateAttributes();
