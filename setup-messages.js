const sdk = require('node-appwrite');

const client = new sdk.Client();
client
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('viscareelojavirtual1610')
    .setKey('standard_');

const databases = new sdk.Databases(client);
const DB_ID = '6a390e430024feb8df57';
const MESSAGES_COL_ID = 'messages';

async function setupMessages() {
    try {
        console.log('Creating messages collection...');
        try {
            await databases.createCollection(DB_ID, MESSAGES_COL_ID, 'Messages', [
                sdk.Permission.create(sdk.Role.any()), 
                sdk.Permission.read(sdk.Role.users()), 
                sdk.Permission.update(sdk.Role.users()), 
                sdk.Permission.delete(sdk.Role.users()), 
            ]);
            console.log('Collection created.');
        } catch(e) { console.log('Collection might exist:', e.message); }

        console.log('Creating attributes...');
        const createAttr = async (fn) => { try { await fn(); } catch(e) { console.log('Attr might exist:', e.message); } };
        
        await createAttr(() => databases.createEmailAttribute(DB_ID, MESSAGES_COL_ID, 'email', true));
        await createAttr(() => databases.createStringAttribute(DB_ID, MESSAGES_COL_ID, 'message', 5000, true));
        await createAttr(() => databases.createStringAttribute(DB_ID, MESSAGES_COL_ID, 'status', 20, false, "new")); // new, read, replied
        console.log('Attributes created/initiated.');

        console.log('Setup finished successfully!');
    } catch(error) {
        console.error('Setup failed', error);
    }
}

setupMessages();
