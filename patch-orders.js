const sdk = require('node-appwrite');

const client = new sdk.Client();
client
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('viscareelojavirtual1610')
    .setKey('standard_');

const databases = new sdk.Databases(client);
const DB_ID = '6a390e430024feb8df57';
const ORDERS_COL_ID = 'orders';

async function patchOrders() {
    try {
        console.log('Adding new attributes to orders collection...');
        const createAttr = async (fn) => { try { await fn(); } catch(e) { console.log('Attr might exist:', e.message); } };
        
        await createAttr(() => databases.createStringAttribute(DB_ID, ORDERS_COL_ID, 'protocolNumber', 255, false));
        await createAttr(() => databases.createStringAttribute(DB_ID, ORDERS_COL_ID, 'paymentMethod', 255, false));
        
        console.log('Done!');
    } catch(error) {
        console.error('Setup failed', error);
    }
}

patchOrders();
