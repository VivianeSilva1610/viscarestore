const sdk = require('node-appwrite');

const client = new sdk.Client();
client
    .setEndpoint('https://fra.cloud.appwrite.io/v1')
    .setProject('viscareelojavirtual1610')
    .setKey('standard_');

const databases = new sdk.Databases(client);
const DB_ID = '6a390e430024feb8df57';
const CATEGORIES_COL_ID = 'categories';

async function setupCategories() {
    try {
        console.log('Creating categories collection...');
        try {
            await databases.createCollection(DB_ID, CATEGORIES_COL_ID, 'Categories', [
                sdk.Permission.read(sdk.Role.any()), 
                sdk.Permission.create(sdk.Role.users()), 
                sdk.Permission.update(sdk.Role.users()), 
                sdk.Permission.delete(sdk.Role.users()), 
            ]);
            console.log('Collection created.');
        } catch(e) { console.log('Collection might exist:', e.message); }

        console.log('Creating attributes...');
        try {
            await databases.createStringAttribute(DB_ID, CATEGORIES_COL_ID, 'label', 255, true);
            await databases.createStringAttribute(DB_ID, CATEGORIES_COL_ID, 'value', 255, true);
            await databases.createStringAttribute(DB_ID, CATEGORIES_COL_ID, 'description', 5000, false);
            console.log('Attributes created/initiated.');
        } catch(e) { console.log('Attributes might exist:', e.message); }

        console.log('Setup finished successfully!');
    } catch(error) {
        console.error('Setup failed', error);
    }
}

setupCategories();
